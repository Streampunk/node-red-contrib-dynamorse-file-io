...
module.exports = function (RED) {
  ...
  function RawFileIn (config) {
    ...
    fsaccess(config.file, fs.R_OK)
      .then(() => {
        var p = null;
        if (config.headers)
          p = fsaccess(config.headers, fs.R_OK);
        if (+config.grainSize <= 0)
          p = Promise.reject(new Error('No headers file and grain size is zero.'));
        return p;
      })
      .then(() => {
        if (config.headers) {
      ...
      .then(tags => {
        node.tags = tags;
        let cableSpec = {};
        cableSpec[this.tags.format] = [{ tags : node.tags }];
        cableSpec.backPressure = `${node.tags.format}[0]`;
        node.makeCable(cableSpec);
        flowID = node.flowID();
        sourceID = node.sourceID();

        return node.highland(
          rawInlet(
            config.file,
            ...
            .map(g => {
              ...
              return new Grain(g.buffers, grainTime,
                ...
                g.timecode, flowID, sourceID, g.duration);
            })
  ...
  RawFileIn.prototype.sdpURLReader = util.promisify(SDPProcessing.sdpURLReaderDynamorse);
